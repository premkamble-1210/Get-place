...
function Popupmodel({ isOpen, onClose, handledata }) {
  ...
  const [newdata, setNewdata] = useState({ roomName: '', isPublic: true, date: '', time: '',Roll:'' });
  const [error, setError] = useState({ roomName: false, date: false, time: false, Roll: false });
  ...
  const handlenewdata = () => {
    if (validateFields()) {
      handledata(newdata.roomName, isPublic, newdata.date, newdata.time,newdata.Roll);
    }
  };
  ...
  const handleTimeChange = (e) => {
    ...
    setNewdata({ ...newdata, Roll: e.target.value });
    if (e.target.value !== '') setError((prev) => ({ ...prev, Roll: false }));
    };
        ...
        <div className="modal rounded-xl shadow-lg md:p-6 w-[90%] md:w-[99%] md:px-20 max-w-4xl mx-auto relative text-sm">
          ...
          <div className="content w-full py-2 px-1">
            ...
            <div className="mb-4">
              ...
              />
              {error.time && <p className="text-red-500 text-xs mt-1">Roll is required.</p>}
            </div>
          </div>
...
function Test3() {
  ...
  const addSchedule = (schedule) => {
    const schedulesRef = ref(database, '1vs1interveiw');
    const newScheduleRef = push(schedulesRef);
    set(newScheduleRef, schedule)
 ...
 function handledata(roomName, isPublic,date,time,Roll) {
      ...
      onValue(schedulesRef, (snapshot) => {
        ...
        if (data) {
          ...
          const dataArray = Object.keys(data)
            ...
            .filter((item) => {
              const meetingDate = new Date(item.date + 'T' + item.time);
              return meetingDate >= now; // Keep only future meetings
            });
          ...
          console.log('Sorted Upcoming Schedules:', sortedData);
  ...
  Prepare, interact, and showcase your skills in this focused and personalized interview setting.
      ...
      <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-2 lg:grid-cols-2">
        {date &&
          Object.values(date).map((schedule) => (
            <Interview key={schedule.id} id={schedule.id} title={schedule.title} isPublic={schedule.isPublic} date={schedule.date} time={schedule.time} owner={"Unknown"} Roll={schedule.Roll}/>
          ))}
          </div>
